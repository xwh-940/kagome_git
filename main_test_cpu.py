...
def fermi_distribution(ek_arr):
    _func = 1/(1+np.exp(ek_arr/T))
    return _func

def trans_q(arr,qxn,qyn):
    'e(k)(128*128) -> e(k+q)(128*128) qxn:0~127'
    return np.roll(arr,shift=(qxn,qyn),axis=(0,1))
trans_q_vec = vec(trans_q,signature='(m,m),(),()->(m,m)')

k_ek, k_a = band_vec(kx_band,ky_band) # 128 128 3, 128 128 3 3
k_ek =  k_ek.transpose((2,0,1)).astype(np.complex128)
k_a =  k_a.transpose((2,3,0,1)).astype(np.complex128)
...
def bare_sus(qxn,qyn):
    ...
    _result = -(_element1 * _element2).sum(axis=(4,5,6,7)) / (kn*kn)
    return np.complex64(_result)
